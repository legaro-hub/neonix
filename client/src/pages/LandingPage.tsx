...
const FEATURES = [
  { icon: CalendarIcon, title: 'Единый календарь', text: 'Планируйте посты для всех каналов в одном окне.', color: '#d4ff3a' },
  { icon: BoltIcon, title: 'Автопостинг', text: 'Публикуйте точно в срок — вручную или по расписанию.', color: '#00e5ff' },
  { icon: LayersIcon, title: 'Несколько каналов', text: 'Telegram, Pinterest, YouTube и Instagram.', color: '#8b5cf6' },
  { icon: ChartIcon, title: 'Аналитика', text: 'Просмотры, реакции и охваты в одном отчёте.', color: '#f472b6' },
  { icon: ShieldIcon, title: 'Безопасно', text: 'Токены шифруются, доступ — только ваш.', color: '#4ade80' },
  { icon: ClockIcon, title: 'Часовые пояса', text: 'Публикация уходит в нужный момент.', color: '#fbbf24' },
];
...
const PLANS = [
  { name: 'Free', price: '0', period: 'навсегда', desc: 'Для пробы', features: ['3 аккаунта', '30 постов / мес', 'Календарь', 'Базовая аналитика'], cta: 'Начать бесплатно', highlight: false },
  { name: 'Pro', price: '790', period: '/мес', desc: 'Для активных авторов', features: ['15 аккаунтов', 'Безлимит постов', 'Массовая загрузка', 'Полная аналитика', 'Приоритетная поддержка'], cta: 'Получить Pro', highlight: true },
  { name: 'Business', price: '2490', period: '/мес', desc: 'Для команд', features: ['До 30 каналов', 'Команда и роли', 'Шаблоны', 'Экспорт отчётов', 'Персональная поддержка'], cta: 'Обсудить', highlight: false },
];
...
export function LandingPage() {
  return (
    <div className="scroll-snap-container">
      ...
      {/* Hero */}
      <section className="scroll-snap-section relative flex items-center">
        <AuroraHero />
        ...
        <div className="container-app relative z-10 py-20 sm:py-28">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <div className="mt-10 flex flex-col items-center justify-center gap-4 sm:flex-row">
              ...
              <Link to="/login" className="btn-ghost btn-lg">Войти в кабинет</Link>
            </div>
          ...
          <div className="relative mx-auto mt-20 h-80 w-80 sm:h-96 sm:w-96">
            ...
            {PLATFORMS.map((p, i) => (
              <div key={p.name} className="absolute top-1/2 left-1/2"
                ...
                <div className="w-12 h-12 -ml-6 -mt-6 rounded-xl flex items-center justify-center text-lg backdrop-blur-sm border border-white/10 bg-white/[0.03]"
      ...
      <section className="scroll-snap-section relative border-y border-graphite-800/50 overflow-hidden flex items-center">
        ...
        <div className="container-app relative grid grid-cols-2 gap-6 py-16 text-center sm:grid-cols-4 w-full">
          {[
            ...
            { v: '24/7', l: 'очередь', icon: '🔄', color: '#8b5cf6' },
          ].map((s) => (
            <div key={s.l} className="group">
              <div className="mb-4 text-3xl transition-transform duration-500 group-hover:scale-125">{s.icon}</div>
          ...
          <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
            {STEPS.map((s, i) => (
              <div key={s.n} className="relative group">
                ...
                <div className="gradient-line my-4 opacity-50 group-hover:opacity-100 transition-opacity" />
      ...
      <section className="scroll-snap-section relative py-24 sm:py-32 flex items-center">
        <div className="container-app w-full">
          <div className="mx-auto max-w-2xl text-center mb-16">
            ...
            <p className="mt-4 text-graphite-400 text-lg">Контент-мейкеры и команды</p>
          </div>
          <div className="grid gap-6 sm:grid-cols-3">
            {[
              { name: 'Алексей К.', role: 'Блогер, 50K', text: 'Экономлю по 3 часа в день. Планирую посты на неделю вперёд.', rating: 5 },
              { name: 'Мария В.', role: 'SMM-агентство', text: 'Управляем 30+ каналами из одного интерфейса.', rating: 5 },
              { name: 'Дмитрий С.', role: 'YouTube + TG', text: 'Кросспостинг работает безупречно. Рекомендую.', rating: 5 },
            ].map((review) => (
              ...
              <Card3D key={p.name} glowColor={p.highlight ? 'rgba(212,255,58,0.15)' : 'rgba(255,255,255,0.03)'}>
                <div className={`card-glass relative flex flex-col p-8 transition-all duration-500 h-full ${
                  ...
                  <MagneticButton className={`mt-8 ${p.highlight ? 'btn-primary btn-lg w-full' : 'btn-ghost btn-lg w-full'}`}>
                    <Link to="/register" className="w-full flex items-center justify-center">{p.cta}</Link>
                  </MagneticButton>
                </div>
              </Card3D>
            ))}
      ...
      <section className="scroll-snap-section container-app py-24 flex items-center">
        <div className="relative overflow-hidden rounded-3xl border border-white/5 p-12 text-center sm:p-20 w-full">
          ...
          <div className="relative mx-auto max-w-2xl z-10">
            ...
            <p className="mt-5 text-graphite-300 text-lg">Создайте аккаунт за полминуты и запланируйте первый пост сегодня.</p>
    ...
    <footer className="border-t border-graphite-800/50 bg-graphite-950">
      <div className="container-app grid gap-8 py-12 sm:grid-cols-2 lg:grid-cols-4">
        <div className="sm:col-span-2 lg:col-span-1">
          <Logo />
          <p className="mt-4 max-w-xs text-sm text-graphite-400 leading-relaxed">Планирование и автопостинг для контент-мейкеров.</p>
