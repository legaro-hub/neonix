...
> MVP-этап. Реализованы: лендинг, регистрация/авторизация, личный кабинет, календарь, редактор постов, массовая загрузка, подключение Telegram-каналов через бота.
...
│       ├── telegram/        # webhook бота, привязка каналов
...
### В планах
- Очередь публикаций (BullMQ + Redis)
- Драйвер Telegram (отправка через Bot API)
